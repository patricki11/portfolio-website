<template>   
    <div ref="internshipComponent" class="row animate-on-scroll exp-edu-row g-4 py-4 row-cols-1 mx-auto align-self-end rounded shadow">
        <span class="fs-6 mt-0 title fw-bold fst-italic">
            {{ title }} {{ $t('employmentAndEducation.internshipAt') }} {{ location }}, {{ city }}
        </span>
        <span class="fs-6 mt-0 sub-date" v-if="education != null">{{ education }}</span>
        <br/>
...
export default {
    ...
    methods: {
        observeElement() {
            const internshipComponent = this.$refs.internshipComponent;
            ...
            const observer = new IntersectionObserver(
                (entries) => {
                    ...
                    if(inView) {
                        internshipComponent.classList.add('animate-in-view')
                    }
                },
                { threshold: 0.1 }
            );

            observer.observe(internshipComponent);
        },
    },
}
